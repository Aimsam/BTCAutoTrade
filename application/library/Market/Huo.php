...
//TODO error log
class Huo extends BaseMarket {
    private $post_param;
    private $api = 'https://api.huobi.com/api.php';
    ...
    public function getInfo() {
        ...
        try {
            ...
            //防止超过火币1秒限制
            if (array_key_exists('frozen_cny_display', $result)) {
                return array(
                    'time' => $timestamp,
                    'frozen_cny' => $result['frozen_cny_display'],
                    'frozen_btc' => $result['frozen_btc_display'],
                    'available_cny' => $result['available_cny_display'],
                    'available_btc' => $result['available_btc_display']
                );
            }
            return null;
        } catch (Exception $e) {
    ...
    public function getPrice() {
        ...
        $curl = new Curl_Huo();
        $curl->setHeader(array("Content-type: application/x-www-form-urlencoded"));
        $content = $curl->get('http://detail.huobi.com/staticmarket/detail.html?jsoncallback=a&_=1393467096690');
        if (empty($content)) {
            return $this->getPrice();
        }
        preg_match("@view_detail\((.*)\)@", $content, $matches);
        if (!isset($matches[1])) {
            return $this->getPrice();
        }
    ...
    function getOrders() {
        $timestamp = time();
        $result = $this->request('get_delegations', $timestamp);
        if (empty($result)) {
    ...
    public function sell($price, $amount) {
        $timestamp = time();
        $result = $this->request('sell', $timestamp, array('price'=>$price, 'amount' => $amount));
        if (empty($result)) {
    ...
    public function buy($price, $amount) {
        $timestamp = time();
        $result = $this->request('buy', $timestamp, array('price'=>$price, 'amount' => $amount));
        if (empty($result)) {
    ...
    public function isDealing($id) {
        sleep(2);
        $timestamp = time();
        $result = $this->request('delegation_info', $timestamp, array('id'=>$id));
        if (empty($result)) {
